...
#region WINDOW
        ...
        public static void ShowWindow() => GetWindow<PsdImportWindow>(true);
        ...
        void OnEnable()
        {
            ...
            _logoTexture = Resources.Load<Texture2D>("Sprites/PsdImportLogo");
            Assert.IsNotNull(_logoTexture);
#if UNITY_2019_1_OR_NEWER
            maxSize = minSize = new Vector2(840, 607);
#else
            maxSize = minSize = new Vector2(840, 611);
#endif
        ...
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.Width(411)))
            {
                if (GUILayout.Button("Browse...", _skin.button))
                {
                    _prevPsdPath = _psdPath;
                    var psdPath = EditorUtility.OpenFilePanel("Select Psd file to import...", Application.dataPath, "psd");
                ...
                {
                    EditorGUILayout.LabelField(System.IO.Path.GetFileName(_psdPath), EditorStyles.helpBox, GUILayout.Height(21), GUILayout.Width(367));
                    var reloadTexture = Resources.Load<Texture2D>("Sprites/reload");
                    if (GUILayout.Button(new GUIContent(reloadTexture, "Reload"), _skin.button))
                    {
                        DestroyAllTextures();
                        LoadFile(_psdPath);
                    }
        ...
        private void PreviewPanel()
        {
            if (_updatePreview)
            {
                ...
                foreach (var item in _hierarchyItems.Values)
                {
                    ...
                    _previewLayers.Add(item.Id, new PreviewLayer(layer.Name, scaledTexture, layer.BlendModeInHierarchy, layer.Visible && layer.VisibleInHierarchy, layer.AlphaInHierarchy));
                }
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.Width(411), GUILayout.Height(206)))
            {
        ...
        private void HierarchyPanel()
        {
            ...
            int h = 546;
#endif
            using (new GUILayout.VerticalScope(EditorStyles.helpBox, GUILayout.Width(411), GUILayout.Height(h)))
            {
                using (var scrollView = new EditorGUILayout.ScrollViewScope(_layerScrollPos, false, true, GUIStyle.none, GUI.skin.verticalScrollbar, GUIStyle.none))
                {
                    _layerScrollPos = scrollView.scrollPosition;
                    using (new GUILayout.VerticalScope(_skin.GetStyle("bottomBorder"), GUILayout.Height(_layerRowHeight * _openItemCount)))
                    {
                        _openItemCount = 0;
        ...
        private void ImportVisiblePanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    GUILayout.Space(4);
                    _importOnlyVisibleLayers = EditorGUILayout.ToggleLeft("Import only visible Layers", _importOnlyVisibleLayers);
                }
            }
        }
        ...
        {
            var labelMiddleRightStyle = new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleRight };
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    _importWidth = Mathf.RoundToInt(EditorGUILayout.FloatField((float)_importWidth / _pixelsPerUnit, textFieldMiddleRightStyle, GUILayout.Width(100)) * _pixelsPerUnit);
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    _importHeight = Mathf.RoundToInt(EditorGUILayout.FloatField((float)_importHeight / _pixelsPerUnit, textFieldMiddleRightStyle, GUILayout.Width(100)) * _pixelsPerUnit);
        ...
        private void AtlasPanel()
        {
            ...
            {
                GUILayout.Space(4);
                _createAtlas = EditorGUILayout.ToggleLeft("Create Atlas", _createAtlas);
                EditorGUIUtility.labelWidth = 100;
                if (_createAtlas) _atlasMaxSize = EditorGUILayout.IntField("Atlas Max Size: ", _atlasMaxSize);
            }
        }
        ...
        private void OutputFolderPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    GUILayout.Space(4);
                    EditorGUILayout.LabelField("Output Folder:", _outputFolder, EditorStyles.helpBox);
        ...
        private void RootObjectPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    GUILayout.Space(10);
                    _importIntoSelectedObject = EditorGUILayout.ToggleLeft("Import into selected object", _importIntoSelectedObject);
                }
            }
        }
        ...
        private void PsComponentsPanel()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                {
                    GUILayout.Space(4);
                    _addPsComponents = EditorGUILayout.ToggleLeft("Add Ps Components:", _addPsComponents);
                    if (_addPsComponents)
                    {
                        EditorGUIUtility.labelWidth = 110;
                        _blendingShader = (PsGroup.BlendingShaderType)EditorGUILayout.Popup("Blending Sahder:", (int)_blendingShader,
        ...
        {
            var importScale = _psdFile.BaseLayer.Rect.width > _psdFile.BaseLayer.Rect.height ? _importWidth / _psdFile.BaseLayer.Rect.width : _importHeight / _psdFile.BaseLayer.Rect.height;
            using (new GUILayout.HorizontalScope())
            {
                if (GUILayout.Button("Import PNG files", _skin.button))
                {
                    var textures = PsLayerCreator.CreatePngFiles(_outputFolder, _layerTextures, _psdFile, importScale, _pixelsPerUnit, _createAtlas, _atlasMaxSize, _importOnlyVisibleLayers);
                }
                if (GUILayout.Button("Create Sprites", _skin.button))
                {
                    if (BlendModeWarning())
                    {
                ...
                if (GUILayout.Button("Create UI Images", _skin.button))
                {
                    if (BlendModeWarning())
                    {
        ...
        private void OnGUI()
        {
            ...
            using (new GUILayout.VerticalScope(GUILayout.MaxWidth(826)))
            {
                GUILayout.Space(4);
                using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                {
                ...
                using (new EditorGUILayout.HorizontalScope(_skin.GetStyle("tableRow"), GUILayout.MinHeight(_layerRowHeight)))
                {
                    using (new EditorGUILayout.VerticalScope(_skin.GetStyle("tableCell"), GUILayout.Width(_layerRowHeight)))
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope(GUILayout.Height(_hierarchyThumbnailH - _layerRowPadding * 2)))
                        {
                            ...
                            layer.Visible = EditorGUILayout.Toggle(layer.Visible, _skin.GetStyle(layer.VisibleInHierarchy ? "visibilityToggle" : "invisibleToggle"), GUILayout.MaxWidth(_hierarchyThumbnailH));
                    ...
                    using (new EditorGUILayout.HorizontalScope(GUILayout.Height(_hierarchyThumbnailH - _layerRowPadding * 2)))
                    {
                        ...
                        using (new EditorGUILayout.VerticalScope(GUILayout.Width(_layerRowHeight), GUILayout.Height(_layerRowHeight)))
                        {
                            ...
                            else
                            {
                                ...
                                GUILayout.Space(4);
                                GUILayout.Label(layerItem.TextureWithBorder, GUILayout.Width(_hierarchyThumbnailH), GUILayout.Height(_hierarchyThumbnailH));
                            }
                        }
                        GUILayout.Space(-8);
                        ...
                        {
                            GUILayout.Space(9);
                            GUILayout.Label(layer.Name, EditorStyles.miniLabel, GUILayout.MaxWidth(186 - indentSize));
                        }
                        using (new EditorGUILayout.VerticalScope(_skin.GetStyle("tableCell"), GUILayout.MinHeight(_layerRowHeight)))
                        {
                            EditorGUILayout.LabelField("Blending: " + ((PsdBlendModeType)layer.BlendModeKey).Name, EditorStyles.miniLabel, GUILayout.MaxHeight(15), GUILayout.Width(117));
                            EditorGUILayout.LabelField("Opacity: " + (int)((float)layer.Opacity / 2.55f) + "%", EditorStyles.miniLabel, GUILayout.MaxHeight(15), GUILayout.Width(117));
                        }
                    }
                }
...
#region LOAD FILE
        ...
        private void OnFileLoaded()
        {
            ...
            _previewRect = GetPreviewRect((int)_psdFile.BaseLayer.Rect.width, (int)_psdFile.BaseLayer.Rect.height, 399, 200);
            _repaint = true;
        }
...
#region LOAD TEXTURES
        ...
        private void LoadPendingTextures()
        {
            ...
            _textureLoader = new TextureUtils.LayerTextureLoader(layer, (int)_psdFile.BaseLayer.Rect.width, (int)_psdFile.BaseLayer.Rect.height, _hierarchyThumbnailW, _hierarchyThumbnailH, (int)_previewRect.width, (int)_previewRect.height);
            _textureLoader.ProgressChanged += OnTextureLoading;
            _textureLoader.OnLoadingComplete += OnTextureLoadingComplete;
        ...
        private void OnTextureLoadingComplete(Layer layer, Color32[] thumbnailPixels, Color32[] thumbnailPixelsWithBorder, Color32[] layerPixels, Rect layerRect)
        {
            _textureLoader.ProgressChanged -= OnTextureLoading;
            _textureLoader.OnLoadingComplete -= OnTextureLoadingComplete;
...
#region PREVIEW IMAGE
        ...
        private struct FloatColor
        {
            ...
            public static implicit operator Color(FloatColor c) => new Color(Mathf.Clamp01(c.r), Mathf.Clamp01(c.g), Mathf.Clamp01(c.b));
        ...
        private static Color GetBlendedPixel(Color backdrop, Color source, float layerAlpha, PsdBlendModeType blendMode)
        {
            ...
            {
                ColorComponent.BlendDelegate Darken = delegate (float bc, float sc)
                {
                    return Mathf.Min(bc, sc);
                };
                return GetBlendResult(backdrop, source, Darken);
            }
            else if (blendMode == PsdBlendModeType.MULTIPLY)
            {
                ColorComponent.BlendDelegate Multiply = delegate (float bc, float sc)
                {
                    return bc * sc;
                };
            ...
            {
                ColorComponent.BlendDelegate Lighten = delegate (float bc, float sc)
                {
                    return Mathf.Max(bc, sc);
                };
            ...
            {
                ColorComponent.BlendDelegate LinearDodge = delegate (float bc, float sc)
                {
                    return bc + sc * source.a;
                };
            ...
            else if (blendMode == PsdBlendModeType.VIVID_LIGHT)
            {
                ...
                {
                    return sc <= 0.5f ?
                    sc == 0f ? 0f : Mathf.Clamp01(1f - (1f - bc) / (2f * sc)) :
                    sc == 1f ? 1f : bc / (2 * (1f - sc));
                };
            ...
            {
                ColorComponent.BlendDelegate LinearLight = delegate (float bc, float sc)
                {
                    return bc + 2f * sc - 1f;
                };
            ...
            {
                ColorComponent.BlendDelegate HardMix = delegate (float bc, float sc)
                {
                    return sc < 1f - bc ? 0f : 1f;
                };
            ...
            {
                ColorComponent.BlendDelegate Difference = delegate (float bc, float sc)
                {
                    return Mathf.Abs(sc - bc);
                };
            ...
            {
                ColorComponent.BlendDelegate Substract = delegate (float bc, float sc)
                {
                    return bc - sc;
                };
            ...
            {
                ColorComponent.BlendDelegate Divide = delegate (float bc, float sc)
                {
                    return sc == 0f ? 1f : bc / sc;
                };
